...
class KDTree
{
  ...
  public:
    ...
    */
    KDTreeNode * buildKDTree(int left, int right, int dim);
    ...
    */
    ...
    Point<Dim> quickSelect(int left, int right, int dim, int pivot);
    ...
    * index.
    */
    int partition(int left, int right, int dim, int pivotIndex);
    /**
    ...
    */
     ...
     */
    void traverseKDTree(Point<Dim> &best_p, KDTreeNode * curr_node, const Point<Dim>&query, int dim) const;
    /*
     * Calculates the radius of two points.
     */
    int find_radius(const Point<Dim>& point1, const Point<Dim>& point2) const;
