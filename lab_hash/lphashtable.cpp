...
int LPHashTable<K, V>::findIndex(const K& key) const
{
     ...
     */
     size_t index = hash(key,size);
     size_t count = index;
     while(should_probe[index]){ // a pair exists in this index
...
void LPHashTable<K, V>::resizeTable()
{
     ...
     */
     ...
     pair<K,V>** new_table = new pair<K,V>*[new_s];
     ...
     for (size_t i=0;i<size;i++){
         if (table[i]!= NULL){
             size_t new_index = hash(table[i]->first,new_s);
             while(new_table[new_index]!=NULL){
                 new_index = (new_index+1)%new_s;
             }
