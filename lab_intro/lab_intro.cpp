...
PNG createSpotlight(PNG image, int centerX, int centerY) {
    //loop through all the pixels of the PNG
    for (unsigned int x = 0; x<image.width(); x++){
        for (unsigned int y = 0; y<image.height(); y++){
            int xDiffsq = pow((x - centerX),2);
            int yDiffsq = pow((y - centerY),2);
            double pixel_away = sqrt(xDiffsq + yDiffsq);
            double percent_dec=(1-((pixel_away*0.5)/100));
            
            if (pixel_away > 160){
                percent_dec = 0.2;
            }
            HSLAPixel & pixel = image.getPixel(x,y);
            pixel.l = pixel.l * percent_dec;
...
PNG watermark(PNG firstImage, PNG secondImage) {
    for (unsigned int x = 0; x<secondImage.width() && x<firstImage.width(); x++){
        for (unsigned int y = 0; y<secondImage.height()  && y<firstImage.height(); y++){
