...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    cap_1.resize(word1.size());
    cap_2.resize(word2.size());
    std::transform(word1.begin(),word1.end(),cap_1.begin(),::toupper);
    std::transform(word2.begin(),word2.end(),cap_2.begin(),::toupper);
    if (dict.find(cap_1) != dict.end()){
        vec_1 = dict.at(cap_1);
        flag1 = true;
    }
    if (dict.find(cap_2) != dict.end()){
        vec_2 = dict.at(cap_2);
        flag2 = true;
    }
